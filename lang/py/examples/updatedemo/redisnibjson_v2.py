...
class RedisNib:
    ...
    def add_node(self, n, attr_dict=None, **attr):
        ...
        >>> nib.add_node('sw1',device='switch',active=True)
        >>> nib.add_node('sw2')
        ...
        if attr:
            ...
            try:
                attr_dict_upd = dict()
                attr_dict_upd['nodeattr'] = attr_dict
            ...
            self.r.set('nodeattr:'+n, json.dumps(attr_dict_upd))
    ...
    def add_edge(self, u, v, attr_dict=None, **attr):
        ...
        if attr:
            ...
            try:
                attr_dict_upd = dict()
                attr_dict_upd['edgeattr'] = attr_dict
            except: # TODO (ks): how to handle errors?
                print 'The attr_dict argument must be a dictionary.'
            # Store the attributes in addition to storing the node
            self.r.set('edgeattr:' + u + '@' + v, json.dumps(attr_dict_upd))
...
def test_add_del_node(nib):
    ...
    assert (json.loads(attr_dict_str)\
        .get('nodeattr').get('device') == \
        'switch'), 'Failed to set node attrib device'
    assert (json.loads(attr_dict_str)\
        .get('nodeattr').get('active') == \
        True), 'Failed to set node attrib active'
...
def test_get_node(nib):
    ...
    assert (nib.node('sw1', 'active') == True), 'Found dangling node attrib'
...
def test_add_del_edges(nib):
    ...
    assert (json.loads(attr_dict_str).get('edgeattr')\
        .get('outport') ==  8675), 'Failed to set edge attrib'
